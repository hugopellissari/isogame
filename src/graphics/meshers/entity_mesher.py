...
class PrimitiveMesher:
    ...
    @staticmethod
    def create_pyramid(size: float = 1.0, height: float = 1.0, color: tuple = (1, 1, 1, 1)) -> MeshData:
        data = MeshData()
        r = size / 2.0
    ...
    @staticmethod
    def create_quad(width: float = 1.0, height: float = 1.0, color: tuple = (1,1,1,1)) -> MeshData:
        """
    ...
    @staticmethod
    def create_cylinder(radius: float = 0.5, height: float = 1.0, segments: int = 8, color: tuple = (1,1,1,1)) -> MeshData:
        """
        ...
        for i in range(segments):
            ...
            data.triangles.extend([cursor, cursor+1, cursor+2, cursor, cursor+2, cursor+3])
            cursor += 4
...
class MeshBuilder:
    ...
    def add_cylinder(self, position=(0,0,0), radius=0.5, height=1.0, segments=8, color=(1,1,1,1)):
        # Cylinders are tricky to scale uniformly, so we usually rely on the primitive params
        # But we still allow position offset
        cyl = PrimitiveMesher.create_cylinder(radius=radius, height=height, segments=segments, color=color)
